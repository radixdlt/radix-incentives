...
export class EnhancedComponentFetcher extends Effect.Service<EnhancedComponentFetcher>()(
  ...
  {
    dependencies: [
      ComponentRepo.Default,
      AssetManager.Default,
      DappConstantsManager.Default,
    ],
    effect: Effect.gen(function* () {
      ...
      return {
        fetchComponentsWithAssetValidation: Effect.fn(function* (
          ...
          const processedComponents = yield* Effect.forEach(
            ...
            Effect.fnUntraced(function* (component) {
              ...
              if ('tvl' in component && Array.isArray(component.tvl)) {
                for (const tvlItem of component.tvl) {
                  if (tvlItem.resourceAddress?.resourceAddress) {
                    const result =
                      yield* assetManager.processTokenAndAddIfMissing(
              ...
              if (['c9', 'oc', 'dp'].includes((component as any).dappId)) {
                console.log(
                  `Adding component to ${(component as any).dappId} constants...`,
                );
                const constantsResult =
                  yield* dappConstantsManager.addComponentToConstants({
              ...
              return {
                ...component,
                _assetAdditions:
                  assetAdditions.length > 0 ? assetAdditions : undefined,
                _constantsAdditions:
                  constantsAdditions.length > 0
                    ? constantsAdditions
                    : undefined,
              };
            }),
