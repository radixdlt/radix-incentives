...
export class AssetManager extends Effect.Service<AssetManager>()('AssetManager', {
  effect: Effect.gen(function* () {
    ...
    const fetchTokenMetadata = Effect.fn(function* (resourceAddress: string) {
      ...
      const symbolItem = metadataItems.find((item: { key: string; value?: { typed?: { value?: string } } }) => item.key === 'symbol');
      if (!symbolItem) {
        return null;
      }
    ...
    const addAssetToFile = Effect.fn(function* (resourceAddress: string, symbol: string) {
      ...
      for (let i = 0; i < lines.length; i++) {
        if (lines[i].includes('  },') && lines[i + 1]?.includes('} as const;')) {
      ...
      if (fungibleInsertIndex === -1) {
        return yield* Effect.fail(new Error('Could not find Assets.Fungible insertion point'));
      }
      ...
      for (let i = 0; i < lines.length; i++) {
        if (lines[i].includes('  // Native Radix assets')) {
          ...
          for (let j = i + 1; j < lines.length; j++) {
            if (lines[j].includes('  },') && lines[j + 1]?.includes('  // Wrapped/bridged assets')) {
      ...
      if (nativeAssetsInsertIndex === -1) {
        return yield* Effect.fail(new Error('Could not find nativeAssets insertion point'));
      }
      ...
      yield* Effect.tryPromise(() => fs.writeFile(assetsFilePath, newContent));

      return `Added ${symbolUpper}: '${resourceAddress}' to Assets.Fungible and '${symbolLower}' to nativeAssets mapping`;
    });

    const processTokenAndAddIfMissing = Effect.fn(function* (resourceAddress: string) {
      if (checkAssetExists(resourceAddress) || addedAssetsInThisSession.has(resourceAddress)) {
        return null; // Asset already exists or was added in this session
      }

      console.log(`Asset ${resourceAddress} not found in Assets.Fungible, fetching metadata...`);
