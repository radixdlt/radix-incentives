...
interface SimpleCategoryBreakdownProps {
  items: Array<{
    id: string;
    name: string;
    value: number;
...
export function SimpleCategoryBreakdown({
  items,
  title,
  formatValue = (value: number) => value.toLocaleString(),
  valueSuffix = '',
}: SimpleCategoryBreakdownProps) {
  ...
  const formatDisplayValue = (value: number) => {
    return formatValue(value) + valueSuffix;
  };
                ...
                return (
                  ...
                  >
                    <div className="-translate-x-1/2 pointer-events-none absolute bottom-full left-1/2 z-10 mb-2 transform whitespace-nowrap rounded bg-black px-2 py-1 text-white text-xs opacity-0 transition-opacity duration-200 group-hover:opacity-100">
                      {item.name}: {formatDisplayValue(item.value)}
                    </div>
                  </button>
                );
              })}
