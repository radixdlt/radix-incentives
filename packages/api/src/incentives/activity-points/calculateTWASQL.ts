...
import { z } from 'zod';
import { DbClientService, DbError, dbReadOnlyClientLive } from '../db/dbClient';
...
export class CalculateTWASQLService extends Effect.Service<CalculateTWASQLService>()(
  'CalculateTWASQLService',
  {
    dependencies: [dbReadOnlyClientLive],
    effect: Effect.gen(function* () {
      const readOnlyDb = yield* DbClientService;
      ...
      return Effect.fn(function* (input: CalculateTWASQLInput) {
        ...
        if (input.addresses.length <= MAX_ADDRESSES_PER_BATCH) {
          return yield* executeQuery(input.addresses);
        }
