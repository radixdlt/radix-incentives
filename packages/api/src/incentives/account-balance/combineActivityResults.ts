...
export type CombinedLendingData = {
  ...
  protocolContributions: Record<
    string,
    LendingProtocolData & { usdValue: string }
  >;
  protocolCount: number;
};

// DEX protocol data types
type CaviarnineProtocolData = {
  protocol: "caviarnine";
  // Add specific fields when needed
};

type DefiPlazaProtocolData = {
  protocol: "defiplaza";
  // Add specific fields when needed
};

type DexProtocolData = CaviarnineProtocolData | DefiPlazaProtocolData;
...
export type CombineActivityResultsOutput = (
...
const activityCombiners = {
  lending: (
    ...
    const protocolContributions: Record<
      string,
      LendingProtocolData & { usdValue: string }
    > = {};
  ...
  provideLiquidityToDex: (
    ...
    const protocolContributions: Record<
      string,
      DexProtocolData & { usdValue: string }
    > = {};

    for (const result of results) {
      // Determine protocol based on data type
      if (
        result.data &&
        typeof result.data === "object" &&
        "type" in result.data
      ) {
        if (result.data.type === "c9_lp") {
          protocolContributions["caviarnine"] = {
            protocol: "caviarnine",
            usdValue: result.usdValue.toString(),
          };
        } else if (result.data.type === "defiplaza_lp") {
          // If protocol already exists, add to its value, otherwise create new entry
          const existingValue =
            protocolContributions["defiplaza"]?.usdValue ?? "0";
          protocolContributions["defiplaza"] = {
            protocol: "defiplaza",
            usdValue: new BigNumber(existingValue)
              .plus(result.usdValue)
              .toString(),
          };
        }
      } else {
        // Fallback for empty data (zero balance cases)
        // Don't add to protocol contributions
      }
    }
    ...
    const combinedData: CombinedDexData = {
      ...
      totalUsdValue: totalUsdValue.toString(),
      protocolContributions,
      protocolCount: Object.keys(protocolContributions).length,
    };
  ...
  Effect.gen(function* () {
    return (input) =>
      Effect.gen(function* () {
        ...
        for (const [_key, groupResults] of groupedResults.entries()) {
          if (groupResults.length === 1) {
            ...
            const combiner =
              activityCombiners[activityId as keyof typeof activityCombiners];
