...
layer(
  SignInWithRolaProofService.Default.pipe(
    Layer.provide(VerifyChallengeService.Default),
  ),
)('signInWithRolaProofService', (it) => {
  beforeEach(async () => {
  ...
  it.effect('should sign in with a valid rola proof', () =>
    Effect.gen(function* () {
      ...
      const result = yield* Effect.tryPromise(() =>
        db.select().from(user).where(eq(user.identityAddress, persona.address)),
