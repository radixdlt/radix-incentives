import type { GetFluxCdpsOutput } from '../getFluxCdps';

export const expectedFluxCdps: GetFluxCdpsOutput = [
  {
    accountAddress:
      'account_rdx12xl2meqtelz47mwp3nzd72jkwyallg5yxr9hkc75ac4qztsxulfpew',
    cdpPositions: [
      {
        nft: {
          resourceAddress:
            'resource_rdx1ntz3kr363rnnf8rjkf7hddf8hrhuu8aqmg5g6ufwxm86anxrqhvtxp',
          localId: '#2#',
        },
        collateralAddress:
          'resource_rdx1thksg5ng70g9mmy9ne7wz0sc7auzrrwy7fmgcxzel2gvp8pj0xxfmf',
        collateralAmount: '399.970724642140443032',
        poolDebt: '1.496906401626036021',
        realDebt: '1.5014780971377246309980354855721879135',
        collateralFusdRatio: '267.198219078805805134',
        interest: '0.25',
        lastInterestChange:
          'Sat May 31 2025 16:04:57 GMT+0200 (Central European Summer Time)',
        status: 'Healthy',
        privilegedBorrower: null,
      },
      {
        nft: {
          resourceAddress:
            'resource_rdx1ntz3kr363rnnf8rjkf7hddf8hrhuu8aqmg5g6ufwxm86anxrqhvtxp',
          localId: '#1#',
        },
        collateralAddress:
          'resource_rdx1tknxxxxxxxxxradxrdxxxxxxxxx009923554798xxxxxxxxxradxrd',
        collateralAmount: '5499.966896520027558611',
        poolDebt: '16.527040089927098323',
        realDebt: '16.56781568457638103293289634771366629088',
        collateralFusdRatio: '332.785959651186893995',
        interest: '0.202',
        lastInterestChange:
          'Sat May 31 2025 16:04:23 GMT+0200 (Central European Summer Time)',
        status: 'Healthy',
        privilegedBorrower: null,
      },
    ],
  },
  {
    accountAddress:
      'account_rdx168nr5dwmll4k2x5apegw5dhrpejf3xac7khjhgjqyg4qddj9tg9v4d',
    cdpPositions: [],
  },
  {
    accountAddress:
      'account_rdx16y4gqnchvxeszcpswg2zldgsle6uqvnl0znerne70tw9535njhkgzk',
    cdpPositions: [
      {
        nft: {
          resourceAddress:
            'resource_rdx1ntz3kr363rnnf8rjkf7hddf8hrhuu8aqmg5g6ufwxm86anxrqhvtxp',
          localId: '#15#',
        },
        collateralAddress:
          'resource_rdx1thksg5ng70g9mmy9ne7wz0sc7auzrrwy7fmgcxzel2gvp8pj0xxfmf',
        collateralAmount: '390000',
        poolDebt: '1750.003144878735036812',
        realDebt: '1750.20120187130759108365914550748274502808',
        collateralFusdRatio: '222.856742367182839925',
        interest: '0.22',
        lastInterestChange:
          'Wed Jun 04 2025 22:29:48 GMT+0200 (Central European Summer Time)',
        status: 'Healthy',
        privilegedBorrower: null,
      },
      {
        nft: {
          resourceAddress:
            'resource_rdx1ntz3kr363rnnf8rjkf7hddf8hrhuu8aqmg5g6ufwxm86anxrqhvtxp',
          localId: '#14#',
        },
        collateralAddress:
          'resource_rdx1tknxxxxxxxxxradxrdxxxxxxxxx009923554798xxxxxxxxxradxrd',
        collateralAmount: '80000',
        poolDebt: '300.001800661072823526',
        realDebt: '300.0305046217977799964132835444658881291',
        collateralFusdRatio: '266.665066088653374163',
        interest: '0.185',
        lastInterestChange:
          'Wed Jun 04 2025 22:28:21 GMT+0200 (Central European Summer Time)',
        status: 'Healthy',
        privilegedBorrower: null,
      },
      {
        nft: {
          resourceAddress:
            'resource_rdx1ntz3kr363rnnf8rjkf7hddf8hrhuu8aqmg5g6ufwxm86anxrqhvtxp',
          localId: '#13#',
        },
        collateralAddress:
          'resource_rdx1tknxxxxxxxxxradxrdxxxxxxxxx009923554798xxxxxxxxxradxrd',
        collateralAmount: '400000',
        poolDebt: '1500.002204082525095785',
        realDebt: '1500.1656435927211967352186787414529140759',
        collateralFusdRatio: '266.666274830349076025',
        interest: '0.19',
        lastInterestChange:
          'Wed Jun 04 2025 21:58:45 GMT+0200 (Central European Summer Time)',
        status: 'Healthy',
        privilegedBorrower: null,
      },
      {
        nft: {
          resourceAddress:
            'resource_rdx1ntz3kr363rnnf8rjkf7hddf8hrhuu8aqmg5g6ufwxm86anxrqhvtxp',
          localId: '#12#',
        },
        collateralAddress:
          'resource_rdx1thksg5ng70g9mmy9ne7wz0sc7auzrrwy7fmgcxzel2gvp8pj0xxfmf',
        collateralAmount: '500000',
        poolDebt: '2121.381877606908035281',
        realDebt: '2123.13158634603778103999269079208660701369',
        collateralFusdRatio: '235.695423477474420609',
        interest: '0.21',
        lastInterestChange:
          'Wed Jun 04 2025 21:57:36 GMT+0200 (Central European Summer Time)',
        status: 'Healthy',
        privilegedBorrower: null,
      },
      {
        nft: {
          resourceAddress:
            'resource_rdx1ntz3kr363rnnf8rjkf7hddf8hrhuu8aqmg5g6ufwxm86anxrqhvtxp',
          localId: '#9#',
        },
        collateralAddress:
          'resource_rdx1thksg5ng70g9mmy9ne7wz0sc7auzrrwy7fmgcxzel2gvp8pj0xxfmf',
        collateralAmount: '182500',
        poolDebt: '779.166485084125579797',
        realDebt: '779.73785247491448596996724147795283312974',
        collateralFusdRatio: '234.224653515860240344',
        interest: '0.24',
        lastInterestChange:
          'Wed Jun 04 2025 00:13:56 GMT+0200 (Central European Summer Time)',
        status: 'Healthy',
        privilegedBorrower: null,
      },
      {
        nft: {
          resourceAddress:
            'resource_rdx1ntz3kr363rnnf8rjkf7hddf8hrhuu8aqmg5g6ufwxm86anxrqhvtxp',
          localId: '#7#',
        },
        collateralAddress:
          'resource_rdx1thksg5ng70g9mmy9ne7wz0sc7auzrrwy7fmgcxzel2gvp8pj0xxfmf',
        collateralAmount: '127707.806042275247437869',
        poolDebt: '499.323904330072669599',
        realDebt: '499.54825653405403041252755227486422077805',
        collateralFusdRatio: '255.761450502989303491',
        interest: '0.12',
        lastInterestChange:
          'Tue Jun 03 2025 18:11:30 GMT+0200 (Central European Summer Time)',
        status: 'Healthy',
        privilegedBorrower: null,
      },
      {
        nft: {
          resourceAddress:
            'resource_rdx1ntz3kr363rnnf8rjkf7hddf8hrhuu8aqmg5g6ufwxm86anxrqhvtxp',
          localId: '#4#',
        },
        collateralAddress:
          'resource_rdx1thksg5ng70g9mmy9ne7wz0sc7auzrrwy7fmgcxzel2gvp8pj0xxfmf',
        collateralAmount: '221018.493889579860996845',
        poolDebt: '799.334590995755150948',
        realDebt: '799.99387951618491526435580073294770197252',
        collateralFusdRatio: '276.50310192913142042',
        interest: '0.21',
        lastInterestChange:
          'Tue Jun 03 2025 16:35:29 GMT+0200 (Central European Summer Time)',
        status: 'Healthy',
        privilegedBorrower: null,
      },
      {
        nft: {
          resourceAddress:
            'resource_rdx1ntz3kr363rnnf8rjkf7hddf8hrhuu8aqmg5g6ufwxm86anxrqhvtxp',
          localId: '#3#',
        },
        collateralAddress:
          'resource_rdx1tknxxxxxxxxxradxrdxxxxxxxxx009923554798xxxxxxxxxradxrd',
        collateralAmount: '249914.382509764802086868',
        poolDebt: '929.87595652538966139',
        realDebt: '930.5335285162815192147830172248071466661',
        collateralFusdRatio: '268.760989846005391967',
        interest: '0.18',
        lastInterestChange:
          'Tue Jun 03 2025 16:34:48 GMT+0200 (Central European Summer Time)',
        status: 'Healthy',
        privilegedBorrower: null,
      },
    ],
  },
];
