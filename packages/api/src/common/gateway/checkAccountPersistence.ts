...
export class CheckAccountPersistenceService extends Context.Tag(
  ...
  CheckAccountPersistenceService,
  (addresses: string[]) => Effect.Effect<
    AccountPersistenceResult[],
  ...
  Effect.gen(function* () {
    ...
    return (addresses: string[]) => {
      return Effect.gen(function* () {
        const results: AccountPersistenceResult[] = [];

        for (const address of addresses) {
          try {
            yield* Effect.logInfo(`Checking persistence for account: ${address}`);
            ...
            const response = yield* Effect.tryPromise({
              try: () =>
                ...
                Effect.logError(`Gateway API call failed for ${address}:`, error);
            ...
            if (!accountData) {
              results.push({ address, isPersisted: false });
              continue;
            }
            ...
            const ownerKeysMetadata = metadata.find(item => item.key === "owner_keys");
            const ownerBadgeMetadata = metadata.find(item => item.key === "owner_badge");
            
            const ownerKeysStateVersion = ownerKeysMetadata?.last_updated_at_state_version || 0;
            const ownerBadgeStateVersion = ownerBadgeMetadata?.last_updated_at_state_version || 0;
            
            // Account is persisted if either owner_keys or owner_badge has state_version > 0
            const isPersisted = ownerKeysStateVersion > 0 || ownerBadgeStateVersion > 0;
            const stateVersion = Math.max(ownerKeysStateVersion, ownerBadgeStateVersion);
            ...
            yield* Effect.logError(`Failed to check persistence for ${address}:`, error);
            results.push({ address, isPersisted: false });
          }
        }
...
export const checkForVirtualAccounts = (
  addresses: string[]
): Effect.Effect<void, VirtualAccountError | CheckAccountPersistenceError, CheckAccountPersistenceService> =>
  Effect.gen(function* () {
    ...
    if (virtualAccounts.length > 0) {
      ...
      if (firstVirtualAccount) {
        yield* Effect.fail(new VirtualAccountError(firstVirtualAccount.address));
      }
    }
  });