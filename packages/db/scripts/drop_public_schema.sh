...
echo "This script will attempt to drop the 'public' schema."

# Ask for confirmation
read -p "Are you absolutely sure you want to drop the 'public' schema and all its contents? This is irreversible. (yes/N) " -r
echo # Move to a new line

# Check the confirmation carefully - only proceed if user types 'yes'
if [[ "$REPLY" != "yes" ]]
then
    echo "Operation cancelled. The 'public' schema was not dropped."
    exit 1
fi
...
echo "The 'public' schema has been dropped."
