...
allCaviarNinePools.push({
  dAppId: DappId.caviarnine,
  poolType: PoolType.DEX,
  tokens: [CaviarNineConstants.LSULP.resourceAddress],
  componentAddress: CaviarNineConstants.LSULP.componentAddress,
});
...
const deriveActivities = (
  ...
  return assets.flatMap((asset) => {
    ...
    const lpActivityId = isSingleTokenPool
      ? `${dAppId}_${action}_${asset.assetType}_${asset.name}`
      : `${dAppId}_${action}_${asset.assetType}_${tokenPair}`;
